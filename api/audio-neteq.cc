...
extern "C" {
    ...
    {   
        ...
        if (!context->neteq_->RegisterPayloadType(encode_type, SdpAudioFormat("l16", sample_rate, channels))) {
            return nullptr;
        }
    ...
    int neteq_get_audio(NeteqContext* context, short* data, int channels)
    {
        if (context) {
